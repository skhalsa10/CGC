...
%%%%% Siri %%%%%
    ...
    \textit{The System Administrator is a actor that admiisters the system and helps make sure everythign stays working}
    \par\noindent\rule{\textwidth}{0.4pt}    
    \begin{itemize}
        \item[]\textbf{Use Case:}                                
            Respond to Alerts
        ...
        \item[]\textbf{Goal in Context:}
            The System Administrator is going about a normal day. The Actor has his scree up that alerts him of any part of the system that eeds attetion.

        \item[]\textbf{Preconditions:}
            The actor is in their office. the system has already been set up and has been running.

        \item[]\textbf{Trigger:}
            The alert screen shows red for ANY part of the system being down or needing attention.

        \item[]\textbf{Scenario:}
            
            \begin{enumerate}
                \item The actor is at their desk checking email, reading an article on the computer, or some other task
                \item The actor hears an alert sound and notices a system reporting an issue.
                \item The actor investigates further and obtains the system type and unique ID.
                \item the important fact here is many different types of systems can go down. this could be the network cable to a camera, it could be the touch screen on a Pay Kiosk going out. It could also be a car reporting the need for maintenance.
                \item If the system issue is a computer or network related issue. the actor will begin the process of fixing the system.
                \item if the system requires maintenance than the actor will inform the approporiate staff about the issue.
                \item once the actor has resolved the issue they will confirm the system now reports green again.
        ...
        \item[]\textbf{Exceptions:}
            ...
            \begin{itemize}
                \item[] redundancy of the CGC itself. does it failover automatically?
            \end{itemize}

        \item[]\textbf{Priority:}
            this is critical to meet on of the objectives of the project to make it maintainable. this should be implemented before software can be released
